...
class InputLogicGenerator(RDLListener):
    ...
    def enter_Reg(self, node: "RegNode") -> None:
        ...
        # Store unpacked dimensions separately for placement after signal name
        for i in array_dimensions:
            self.vector_text += (
                f"[{i-1}:0] "  # Keep for backward compatibility during transition
            )
            self.unpacked_dims += (
                f" [{i-1}:0]"  # For after signal name (with leading space)
            )
            self.vector *= i
        ...
        if self.policy.is_external(node):
            ...
            if node.is_sw_readable:
                ...
                self.hwif_port.append(
                    ...
                    if packed_dim
                    else f"input wire {rd_data_name}{self.unpacked_dims}"
                )
            if node.is_sw_writable:
                ...
                if is_single_field:
                    self.hwif_port.append(
                        ...
                        if packed_dim
                        else f"output logic {x}_wr_data{self.unpacked_dims}"
                    )
                    self.hwif_port.append(
                        f"output logic {packed_dim} {x}_wr_biten{self.unpacked_dims}"
                        if packed_dim
                        else f"output logic {x}_wr_biten{self.unpacked_dims}"
                    )
                else:
                    self.hwif_port.append(
                        f"output logic {packed_dim} {x}_wr_data_{node.inst_name}{self.unpacked_dims}"
                        if packed_dim
                        else f"output logic {x}_wr_data_{node.inst_name}{self.unpacked_dims}"
                    )
                    self.hwif_port.append(
                        f"output logic {packed_dim} {x}_wr_biten_{node.inst_name}{self.unpacked_dims}"
                        if packed_dim
                        else f"output logic {x}_wr_biten_{node.inst_name}{self.unpacked_dims}"
                    )
        else:
            if self.hwif.has_value_input(node):
                ...
                if node.get_property("next") is None:
                    ...
                    self.hwif_port.append(
                        ...
                        if packed_dim
                        else f"input wire {input_identifier}{self.unpacked_dims}"
                    )
            if self.hwif.has_value_output(node):
                ...
                self.hwif_port.append(
                    ...
                    if packed_dim
                    else f"output logic {output_identifier}{self.unpacked_dims}"
                )
            ...
            for prop in implied_props:
                ...
                self.hwif_port.append(
                    ...
                    if prop_packed_dim
                    else f"input wire {prop_identifier}{self.unpacked_dims}"
                )
    ...
    def enter_Signal(self, node: "SignalNode") -> None:
        ...
        signal_packed_dim = f"[{width-1}:0]" if width > 1 else ""
        input_identifier = self.hwif.get_input_identifier(node, index=False)
        self.hwif_port.append(
            f"input wire {signal_packed_dim} {input_identifier}{self.unpacked_dims}"
            if signal_packed_dim
            else f"input wire {input_identifier}{self.unpacked_dims}"
        )
