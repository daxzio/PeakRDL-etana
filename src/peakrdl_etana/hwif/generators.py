...
class InputLogicGenerator(RDLListener):
    ...
    def enter_Field(self, node: "FieldNode") -> None:
        ...
        if node.external:
            if node.is_sw_readable:
                in_port.append(
                    f"input wire {field_text} {self.hwif.get_external_rd_data2(node)}"
                )
            ...
            if self.first_read:
                ...
                out_port.append(
                    f"output logic [{self.vector-1}:0] {vector_extend}{out_id}_req"
                )
                out_port.append(f"output logic [{self.vector-1}:0] {out_id}_req_is_wr")
                if node.is_sw_readable or self.regfile:
                    in_port.append(
                        f"input wire {self.vector_text}{self.hwif.get_external_rd_ack(node.parent)}"
                    )
                    self.first_read = False
            if self.first_write:
                if node.is_sw_writable or self.regfile:
                    in_port.append(
                        f"input wire {self.vector_text}{self.hwif.get_external_wr_ack(node.parent)}"
                    )
                    self.first_write = False

            if node.is_sw_writable or self.regfile:
                out_port.append(
                    f"output logic {field_text} {out_id}_{node.inst_name}_wr_data"
                )
                out_port.append(
                    f"output logic {field_text} {out_id}_{node.inst_name}_wr_biten"
                )
        else:
            if self.hwif.has_value_input(node):
