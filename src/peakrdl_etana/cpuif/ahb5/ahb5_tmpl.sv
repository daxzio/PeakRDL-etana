// AHB5 slave interface for shared-bus fabrics (e.g. hazard3 ahbl_splitter).
// Uses split HREADY: HREADYIN (hready input) and HREADYOUT (hready_resp output).
// HSEL is present for spec compliance; tie high when no per-slave decoder exists.
// HBURST, HPROT, HMASTLOCK, HNONSEC, HMASTER are accepted but unused for CSR access.
...
// Local exclusive monitor (AHB5 HEXCL/HEXOKAY)
logic excl_valid;
logic [{{cpuif.addr_width-1}}:0] excl_addr;
logic [3:0] excl_master;
...
always {{get_always_ff_event(cpuif.reset)}} begin
    if({{get_resetsignal(cpuif.reset)}}) begin
        ...
        // Establish exclusive reservation on successful exclusive read
        if (pop_now && txn_valid && !txn_write && txn_hexcl) begin
            excl_valid <= 1'b1;
            excl_addr <= txn_addr;
            excl_master <= {{cpuif.signal("hmaster")}};
        end

        // Any exclusive write attempt consumes the reservation
        if (txn_valid && txn_write && txn_hexcl && txn_data_ready) begin
            excl_valid <= 1'b0;
        end

        // Normal write to reserved address invalidates the monitor
        if (pop_now && txn_valid && txn_write && !txn_hexcl && txn_addr == excl_addr) begin
            excl_valid <= 1'b0;
        end

        cpuif_req <= txn_inflight_next && !(txn_write_next && txn_hexcl_next && !excl_write_ok);
        cpuif_req_is_wr <= txn_inflight_next && txn_write_next && !(txn_hexcl_next && !excl_write_ok);
...
always @(*) begin
    ...
    // Failed exclusive writes complete once data phase is captured (no cpuif ack)
    if (txn_valid && txn_write && txn_hexcl && txn_data_ready && !txn_inflight) begin
        pop_now = 1'b1;
    end
    slot_free = pop_now || !txn_valid;
end

// Exclusive write check (evaluated when HWDATA is captured, before txn_data_ready)
always @(*) begin
    excl_write_ok = 1'b0;
    excl_write_fail = 1'b0;
    if (txn_valid && txn_write && txn_hexcl && !txn_data_ready) begin
        if (excl_valid && (txn_addr == excl_addr) && ({{cpuif.signal("hmaster")}} == excl_master)) begin
            excl_write_ok = 1'b1;
        end else begin
            excl_write_fail = 1'b1;
        end
    end
end
...
always @(*) begin
    ...
    // Address phase qualified by HREADYIN (global bus ready)
    accept_new = {{cpuif.signal("hsel")}} && ({{cpuif.signal("htrans")}} == HTRANS_NONSEQ || {{cpuif.signal("htrans")}} == HTRANS_SEQ) && {{cpuif.signal("hready")}};
    ...
    if (txn_valid && txn_write && !txn_data_ready) begin
        ...
        if (!txn_hexcl || excl_write_ok) begin
    ...
    // Handle completion from the CPU interface (or failed exclusive write)
    if (pop_now) begin
    ...
    if (accept_new) begin
        ...
        if (slot_free) begin
            ...
            txn_hexcl_next = {{cpuif.signal("hexcl")}};
...
// Stall conditions: HREADYOUT low when bus cannot complete or accept new address
...
// Failed exclusive write completes in one cycle once data is captured
assign hready_int    = !(waiting_data || waiting_ack) || (txn_valid && txn_write && txn_hexcl && txn_data_ready && !txn_inflight);
...
assign {{cpuif.signal("hready_resp")}} = hready_int;
assign {{cpuif.signal("hrdata")}} = read_data_extracted;
assign {{cpuif.signal("hresp")}} = (cpuif_rd_err | cpuif_wr_err) ? HRESP_ERROR : HRESP_OKAY;
assign {{cpuif.signal("hexokay")}} = (txn_valid && txn_write && txn_hexcl && txn_data_ready && !txn_inflight) ? 1'b0 : 1'b1;
